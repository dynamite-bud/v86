...
3. **EDID, display events, and resize.** Core device support is complete:
   `GET_EDID`, `VIRTIO_GPU_F_EDID`, `events_read`/`events_clear`, display-change
   config interrupts, snapshot state, and `V86.virtio_gpu_set_size` are covered
   by protocol tests. Add canvas/scanout resize acceptance for both browser
   renderers before closing Phase 5.
...
Phase 5 exits only when `make virtio-gpu-unit-test`, `make virtio-gpu-test`,
`make virtio-gpu-test-release`, and `make virtio-gpu-browser-test` protect the
current 2D desktop path and a malformed guest cannot crash or indefinitely
stall the browser.
