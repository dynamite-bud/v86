 ...
 * @param {number=} total_size Capacity reported through 9p statfs.
 */
export function FS(storage, qidcounter, total_size) {
    /** @type {Array.<!Inode>} */
    this.inodes = [];
    ...
    this.total_size = total_size === undefined ? 256 * 1024 * 1024 * 1024 : total_size;
    if(!Number.isSafeInteger(this.total_size) || this.total_size < 0)
    {
        throw new TypeError("Filesystem total_size must be a non-negative safe integer");
    }
    this.used_size = 0;
...
FS.prototype.CloseInode = async function(id) {
    ...
    {
        this.storage.release(inode.sha256sum);
    }
...
FS.prototype.ChangeSize = async function(idx, newsize)
{
    ...
    // The resized buffer is a copy. Keep the immutable source cached for
    // other inodes that share its content hash.
    if(inode.status === STATUS_ON_STORAGE)
    {
