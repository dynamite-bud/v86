...
export class JsWebGpuRenderer
{
    ...
    upload_resource_2d(resource_id, x, y, width, height, stride, data)
    {
        ...
        let upload_data;
        let bytes_per_row;
        if(rect.height === 1 || stride % COPY_BYTES_PER_ROW_ALIGNMENT === 0)
        {
            upload_data = data.byteLength === required_length ?
                data : data.subarray(0, required_length);
            bytes_per_row = stride;
        }
        else
        {
            bytes_per_row = align_to(row_bytes, COPY_BYTES_PER_ROW_ALIGNMENT);
            const upload_length = checked_multiply(bytes_per_row, rect.height,
            ...
            for(let row = 0; row < rect.height; row++)
            {
                ...
                const target_offset = row * bytes_per_row;
