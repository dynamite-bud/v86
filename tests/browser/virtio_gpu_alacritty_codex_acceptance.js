#!/usr/bin/env node
/* global getComputedStyle, GPUDevice, GPUQueue */
...
const SCENARIO = process.env.V86_CODEX_BROWSER_SCENARIO || "alacritty";
assert.ok(["appliance", "alacritty", "triangle", "shader"].includes(SCENARIO),
    `Invalid V86_CODEX_BROWSER_SCENARIO: ${SCENARIO}`);
const renderers = (process.env.V86_CODEX_BROWSER_RENDERERS ||
    (SCENARIO === "appliance" || SCENARIO === "alacritty" ? "webgpu-js,wgpu" : "wgpu"))
    .split(",")
...
const appliance_image = SCENARIO === "alacritty" ?
    "images/alpine-virtio-gpu-alacritty-codex-fs.json" :
...
async function run_scenario(browser_ws, base_url, renderer)
{
    ...
    try
    {
        ...
        const page = SCENARIO === "alacritty" ?
            "virtio_gpu_alacritty_codex.html" : "virtio_gpu_codex.html";
        const url = new URL(
            `${base_url}/examples/${page}?renderer=${renderer}&acceptance=${Date.now()}`);
        ...
        await wait_for(async() => {
            ...
            {
                const reason = /V86_(?:APPLIANCE|ALACRITTY_CODEX)_FAILURE=([^\r\n]+)/.exec(
                    state.serial)?.[1] || "unknown";
        ...
        if(SCENARIO === "alacritty")
        {
            assert.match(state.serial, /V86_ALACRITTY_CODEX_RENDERER=.*llvmpipe/i);
            assert.match(state.serial, /V86_ALACRITTY_CODEX_OPENGL=4\.[1-9]/);
            assert.match(state.serial, /V86_ALACRITTY_CODEX_ALACRITTY=alacritty 0\.17\.0/);
            assert.ok(state.serial.includes("V86_ALACRITTY_CODEX_CODEX=codex-cli 0.147.0"));
        }
        else
        {
            ...
            await wait_for(async() => {
                ...
                if(reset_state.fatal || reset_state.result === "fail")
                {
                    const reason = reset_state.fatal ||
                        /V86_(?:APPLIANCE|ALACRITTY_CODEX)_FAILURE=([^\r\n]+)/.exec(
                            reset_state.serial)?.[1] ||
                        "unknown";
                    const error = new Error(
        ...
        return {
            ...
            ready_ms: Math.round(ready_ms),
            architecture: "i686",
            terminal: SCENARIO === "alacritty" ? "alacritty" : "ghostty",
            uid: 1000,
            llvmpipe: true,
