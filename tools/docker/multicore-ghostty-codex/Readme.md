# Multi-core Ghostty + Codex Appliance

The **SMP line's own** Ghostty/Codex appliance: the fixture the XWAH-9 worker-per-vCPU work is demonstrated and tested against. It boots Codex inside a maximized, undecorated Ghostty window on **N vCPUs**, each running in its own WebWorker over shared guest memory, and it refuses to report success unless the guest actually brought every vCPU online and ran them concurrently.

## Why this exists separately

It is a sibling of [`../virtio-gpu-alpine-codex`](../virtio-gpu-alpine-codex), not a replacement, and the separation is load-bearing rather than cosmetic:

- **`images/` is shared.** It is gitignored and, in this project's layout, a symlink to the primary checkout, so *every* worktree resolves the same image names. While both appliances built into `alpine-virtio-gpu-codex-*`, whichever branch ran its build script last silently won, and the other branch's page booted a guest it had never described — diagnosed once from an unexpected `python3`/`jq` in the readiness output and a DHCP failure the multi-core contract does not even require. This directory builds `multicore-ghostty-codex-*` and the guest announces `V86_APPLIANCE_IMAGE=multicore-ghostty-codex` in its first lines of serial output, so the mix-up is visible immediately and asserted by the acceptance test.
- **The branches have diverged.** The single-core appliance on `main` carries the XWAH-5/XWAH-6 additions — guest Mesa `webgpuvirt`, `accelerated=1`, the terminal benchmark, pinned Python and jq for MCP workflows. None of that belongs to the SMP work, and this fixture deliberately does not carry it.

Keep both runnable side by side. Anything added here that is not about multi-core execution probably belongs in the sibling instead.

Start with [multicore.md](../../../docs/multicore.md) (worker execution and the run recipe), then the [GPU contributor guide](../../../docs/gpu/README.md) and the [canonical VirtIO GPU architecture](../../../docs/virtio-gpu-webgpu.md).

## What this appliance proves that the single-core one does not

The single-core contract is satisfied by a guest that reaches a shell and paints a window. That is not sufficient evidence for SMP work, so this one adds:

| Marker | Evidence |
| --- | --- |
| `V86_APPLIANCE_IMAGE` | which rootfs actually booted — the guard against the shared-`images/` collision above |
| `V86_APPLIANCE_CPUS` | vCPUs Linux brought **online**: SeaBIOS's INIT-SIPI-SIPI started every AP and the scheduler picked it up |
| `V86_APPLIANCE_CPUS_EXPECTED` | the count the page configured, taken from `v86_cpus=N` on the kernel command line; a mismatch **fails the boot** instead of quietly halving the machine |
| `V86_APPLIANCE_PARALLEL_SPEEDUP` | measured in-guest: *n* units of identical CPU-bound work run concurrently, priced against one unit run alone. Time-sliced execution cannot exceed ~1.0 no matter how the host is loaded, so this separates real parallelism from a machine that merely reports 4 CPUs |
| `V86_APPLIANCE_CODEX_TUI` | Codex is alive, owns Ghostty's pty on **both** stdin and stdout, and has been scheduled — strictly more than `pidof codex`, which stays true for a process that never took the terminal |

The speedup is *graded*, not fatal (`PASS` / `WEAK`): a saturated host can depress it without anything being wrong with the guest. The acceptance test applies the bar.

None of these prove pixels — the guest cannot inspect its own framebuffer. That is the acceptance test's job; see [Verification](#verification).
...
```sh
make multicore-ghostty-codex-image
make all-debug
...
- `multicore-ghostty-codex-rootfs.tar`
- `multicore-ghostty-codex-fs.json`
- `multicore-ghostty-codex-rootfs-flat/`
- `multicore-ghostty-codex-image-contract.json`
...
```text
http://127.0.0.1:8082/examples/multicore_ghostty_codex.html?renderer=webgpu-js
```

Online launch with a percent-encoded WISP/wsproxy relay:

```text
http://127.0.0.1:8082/examples/multicore_ghostty_codex.html?renderer=webgpu-js&relay=wss%3A%2F%2Frelay.example.test%2F
```
