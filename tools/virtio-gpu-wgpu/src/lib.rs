...
impl Renderer {
    ...
    fn flush(&mut self, resource_id: u32, flush_rect: Rect) -> Result<bool, String> {
        ...
        let Some((surface_texture, suboptimal)) = self.acquire_surface_texture()?
        else {
            self.queue.submit([]);
            return Ok(false);
        };
...
fn js_error(message: impl ToString) -> JsValue { js_sys::Error::new(&message.to_string()).into() }
